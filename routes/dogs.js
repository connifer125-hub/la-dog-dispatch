  ...
  try {
    const result = await db.query(`
      SELECT 
        ...
        photo_crop_offset,
        COALESCE(photo_crop_zoom, 1.0) as photo_crop_zoom,
...
router.get('/:id', async (req, res) => {
  try {
    const result = await db.query(
      `SELECT *, rescue_only, intake_date, list_date, photo_crop_offset, COALESCE(photo_crop_zoom, 1.0) as photo_crop_zoom FROM dogs WHERE id = $1`,
      [req.params.id]
    );
